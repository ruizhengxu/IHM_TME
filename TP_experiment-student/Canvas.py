		...
		if self.state == 0: # attention
			...
			for i in range(0, self.mat_size):
				for j in range(0, self.mat_size):
					...
					img = QImage('./ressources/'+ s + '.png') #TODO

					p.drawImage(QPointF(x + (h-img.width()) / 2 ,y + (h-img.height()) / 2), img)
			...
			for i in range(0, self.mat_size):
				for j in range(0, self.mat_size):
					...
					#p.drawRect(x, y, h, h)
					#p.drawText(QPoint(x, y), s)
					img = QImage('./ressources/white-square.png') #TODO
		...
		if self.state ==1:
			...
			for i in range(0, self.mat_size):
				for j in range(0, self.mat_size):
					...
					y = i*(h+offset) + offset
					img = QImage('./ressources/white-square.png') # TODO
					rect = QRectF(x + (h-img.width()) / 2, y + (h-img.height()) / 2, img.width(), img.height())
					#print(rect, e.pos() )
